            ...
            <tbody className="divide-y divide-goldPrimary/10">
              {withdrawals.map((w) => (
                <tr key={w.id}>
                  ...
                  <td className="p-3 font-mono text-amber-400 font-bold">{w.accountNumber}</td>
                  <td className="p-3 font-extrabold text-statusPurple">৳{parseFloat(w.amount).toFixed(2)}</td>
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${
                      w.status === 'APPROVED' ? 'bg-activeGreen/10 text-activeGreen' : w.status === 'REJECTED' ? 'bg-lockedRed/10 text-lockedRed' : 'bg-amber-500/10 text-amber-400'
                    }`}>
                      {w.status}
                    </span>
                  </td>
