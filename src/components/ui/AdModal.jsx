...
export default function AdModal({ isOpen, onClose, onComplete, adUrl }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0a0e17]/80 backdrop-blur-sm">
      <div className="w-full max-w-[360px] bg-navyCard border border-amber-500/50/30 rounded-2xl p-5 shadow-2xl relative">
        <button onClick={onClose} className="absolute top-3 right-3 text-gray-400 hover:text-white p-1">
          <X className="w-5 h-5" />
        </button>

        <div className="text-center">
          <div className="w-12 h-12 rounded-full bg-amber-500/10 border border-amber-500/50/30 flex items-center justify-center mx-auto mb-3">
          ...
          {!adOpened ? (
            ...
            <div className="space-y-3">
              <div className="bg-darkBg/80 p-3 rounded-xl border border-amber-500/50/20">
